...
class P2PNode(Node):
    ...
    def _handle_file_chunk(self, peer_id, payload):
        if peer_id not in self.receiving_files:
    ...
    def get_peer_username(self, peer_id: str) -> str:
        return self.peer_usernames.get(peer_id, "Unknown")
    ...
    def connect_to_peer(self, host: str, port: int) -> bool:
        try:
            print(f"[INFO] Menghubungkan ke - {host}:{port}...")
            ...
            print(f"[ERROR] Gagal terhubung ke - {host} : {port}. Error: {e}")
            return False
    ...
    #  ===== Group Method =====
    def create_group(self, group_id: str, group_name: str, member_ids: str, group_key: bytes):
        self.groups[group_id] = Group(
            group_id=group_id,
            group_name=group_name,
            creator_id=self.id,
